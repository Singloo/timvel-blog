...
const loopDir = (pth, filename = '') => {
  ...
  return flattenDeep(filenames.map((fname) => loopDir(pth, path.join(filename, fname))));
};
...
(async () => {
  ...
  await from(filenames)
    .pipe(mergeMap(({ pth, filename }) => client.put(filename, path.join(pth, filename))))
    .toPromise();
  console.log('DONE');
})();
